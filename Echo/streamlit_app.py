"""Streamlit interface for Echo daily mental health check-ins."""
...
LOW_MOOD_THRESHOLD = 2.8
LOW_MOOD_STREAK_DAYS = 10
MOOD_TO_COLOR = {
	"Very low": "#d62828",
	"Low": "#f77f00",
	"Neutral": "#8d99ae",
...
def _consecutive_low_mood_days(daily_trend: pd.DataFrame, *, threshold: float = LOW_MOOD_THRESHOLD) -> int:
...
st.set_page_config(page_title="Echo | Daily Check-ins", page_icon="📝", layout="wide")
...
if st.session_state.checkins:
	...
	if not daily_df.empty:
		...
		low_streak = _consecutive_low_mood_days(daily_df, threshold=LOW_MOOD_THRESHOLD)
		latest_day = daily_df["date"].max()
		recent_enough = isinstance(latest_day, date) and (date.today() - latest_day) <= timedelta(days=2)
		if recent_enough and low_streak >= LOW_MOOD_STREAK_DAYS:
			st.warning(
				f"Your daily mood average has been {LOW_MOOD_THRESHOLD:.1f} or lower for at least {LOW_MOOD_STREAK_DAYS} days. "
